...
void LevonychevIRadixBatcherSortSTL::RadixSortSequential(std::vector<int>& arr) {
    ...
    for (int byte_idx = 0; byte_idx < 4; ++byte_idx) {
        ...
        int offsets[256];
        offsets[0] = 0;
        for (int i = 1; i < 256; ++i) offsets[i] = offsets[i-1] + count[i-1];
...
void LevonychevIRadixBatcherSortSTL::MergeAndSplit(std::vector<int>& left_block, std::vector<int>& right_block) {
    ...
    std::merge(left_block.begin(), left_block.end(),
               right_block.begin(), right_block.end(),
...
bool LevonychevIRadixBatcherSortSTL::RunImpl() {
    ...
    if (n <= 1) { GetOutput() = data; return true; }

    int num_blocks = 1;
    int max_threads = std::thread::hardware_concurrency();
    while (num_blocks * 2 <= max_threads) num_blocks *= 2;
    ...
    for (int i = 0; i < num_blocks; ++i) {
        futures.push_back(std::async(std::launch::async, [&blocks, i]() {
            RadixSortSequential(blocks[i]);
        }));
    }
